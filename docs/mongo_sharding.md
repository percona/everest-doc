...
```sh
MONGODB_CLUSTER_ADMIN_USER=$(kubectl get secrets --namespace <NAMESPACE> everest-secrets-<CLUSTER_NAME> -o template= --raw'{{ .data.MONGODB_CLUSTER_ADMIN_USER | base64decode }}')
```

```sh
MONGODB_CLUSTER_ADMIN_PASSWORD=$(kubectl get secrets --namespace <NAMESPACE> everest-secrets-<CLUSTER_NAME> -o template= --raw '{{ .data.MONGODB_CLUSTER_ADMIN_PASSWORD | base64decode }}')
```
