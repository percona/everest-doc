...
|----------|-----------|-------------|--------------------|
| **1**|**No downtime**  |Delete the locks. |**a.** Connect to your MongoDB database. </br></br> **b.** Run the command: `db.getSiblingDB("admin").pbmLock.find()` to see the list of database locks. If the list is empty, the scenario is not applicable. </br></br> **c.** If the list was not empty, run the command`db.pbmLock.deleteMany({})`. </br></br> **d.** Run another backup. If the backup still fails, check the workaround 2.|
...
| **3**|**Longer downtime**|Restart the DB server |**a.** On Percona Everest UI, click **Actions >> Restart**. </br></br> **b.** When the database cluster is up, take another backup.|
