...
- [EVEREST-814](https://perconadev.atlassian.net/browse/EVEREST-814) - When Percona Everest was reinstalled, no login token was displayed. To log in, the token from the previous installation was required. Now, the uninstall command also removes the token, so a new token is generated in a subsequent installation.
