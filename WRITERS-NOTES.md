...
We use the `mkdocs-macros` plugin for variable expansion. For example, the variable `release` in `variables.yml` is used in the code so that the current OpenEverest release number is always up-to-date. (Search the markdown files for `{{release}}`.)
